...
const dataFromServer = async () => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const fetchForDelete = async () => {
      const fet = await fetch("http://localhost:3000/users", {
        method: "DELETE",
        body: JSON.stringify({
...
btn.addEventListener("click", () => {
  ...
  const fileuplaod = async () => {
    const fetchForFileupload = await fetch("http://localhost:3000/fileupload", {
      method: "POST",
      body: input.files[0],
    });
  ...
  const fetchForUpdate = async () => {
    const fetcH = await fetch("http://localhost:3000/users");
